...
def get_protein_files(pdb_ids, output_dir, format):
    ...
    for pdb_id in pdb_ids:
        ...
        if protein_file.exists():
            ...
            try:
                if format == 'pdb':
                    urllib.request.urlretrieve('http://ftp.ebi.ac.uk/pub/databases/rcsb/pdb-remediated/data/structures/divided/pdb/cr/pdb' + pdb_id + '.ent.gz',
                                               output_dir + '/' + pdb_id + '.ent.gz')
                elif format == 'fasta':
                    urllib.request.urlretrieve('https://www.ebi.ac.uk/pdbe/entry/pdb/' + pdb_id + '/fasta',
...
if __name__ == '__main__':
    # input file
    # include pdb ids
    input_file = '../input/uniprot_sample.pdbid.list'

    # output dir to store PDB files
    pdb_output_dir = '../output/uniprot/pdb'
    # output dir to store fasta files
    fasta_output_dir = '../output/uniprot/fasta'
